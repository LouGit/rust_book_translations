...
impl CodeTranslator {
    pub fn new(po_file: &Path) -> Result<Self> {
        ...
        for message in catalog.messages() {
            // Avoid conflicts with gettext:
            if message.msgid().contains('\n') {
                continue; // Ignore multiline blocks
            }
...
impl Preprocessor for CodeTranslator {
    ...
    fn run(&self, _ctx: &PreprocessorContext, mut book: Book) -> Result<Book> {
        let mut count_items = 0;   // DEBUG
        let mut count_chapter = 0; // DEBUG
        for item in &mut book.items {
            count_items += 1; // DEBUG
            // println!("-"); // DEBUG
            if let BookItem::Chapter(chapter) = item {
                /*
                println!("CHAPTER: \n {}", chapter.content); // DEBUG
                // DEBUG
                for sub in &chapter.sub_items {
                    if let BookItem::Chapter(sub_ch) = sub {
                        println!("  SUB: {:?}", sub_ch.source_path);
                    }
                }
                count_chapter += 1; // DEBUG
                // DEBUG
                ...
                // println!("|"); //DEBUG
                // DEBUG /*
                if chapter.content.contains("THREE_HOURS") {
                    println!("FOUND in RAW CHAPTER:\n{}", chapter.content);
                }
                println!("CHAPTER: {:?}", chapter.source_path);
                // DEBUG */
        ...
        println!("==================================================="); //DEBUG
        println!("count_items: {count_items}"); //DEBUG
        println!("count_chapter: {count_chapter}"); //DEBUG
        println!("==================================================="); //DEBUG
        Ok(book)
    }
}
